...
void * pixCalc(void * threadIndex)
{
    ...
    int start=(int)threadIndex * pieceSize;
    int end=(int)threadIndex * pieceSize + pieceSize;
    if ((int)threadIndex==nthreads-1)
    {
        end+=extraPiece;
    }
    for( int px=start; px<end; ++px ) //divide jobs here
    {
...
main( int argc, char **argv )
{
    ...
    pieceSize=width / nthreads;
    extraPiece=width % nthreads;
    ...
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE); //can be joined, without this the system won't let you wait for it
        int j,k,rc,l,m;
        for (l=0;l<nthreads;l++)
        {
            rc=pthread_create(&threads[l], &attr, pixCalc, (void *)l);
        }
        ...
        for (j=0;j<width;j++)
        {
            ...
            {
                printf( "%.0f %.0f %.0f\n",
                scaled_color[j][k][0], scaled_color[j][k][1], scaled_color[j][k][2] ); //order matters here, don't print in the parallel, store them somewhere
            }
            printf( "\n" );
        }
